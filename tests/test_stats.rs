...
fn test_stats<S: Str>(name: S, field: &str, rows: &[&str], expected: &str,
    ...
    assert_eq!(field_val, expected.into_string());
}
...
fn setup<S: Str>(name: S, rows: &[&str],
    ...
    let mut data: Vec<Vec<String>> =
        rows.iter().map(|s| vec![s.into_string()]).collect();
...
fn get_field_value(wrk: &Workdir, cmd: &mut process::Command, field: &str)
    ...
    panic!("BUG: Could not find field '{}' in headers '{}' for command '{}'.",
           field, headers, cmd);
}
...
stats_tests!(stats_median_even_null, "median", &["", "1", "2", "3", "4"], "2.5")
